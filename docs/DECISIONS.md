...
## 2026-07-13: Use HLSL-Style Project-Owned Math Values
...
Decision: Publish project-owned `floatN`, `intN`, and `uintN` vectors; matching square matrices; and a single `color`
type for RGBA storage. Use free math functions and `swizzle<...>` rather than vector member functions or
