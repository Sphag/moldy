...
- `install-tools.ps1`: checks selected local C++ quality tools and provides an explicit Windows install path.
...
The install path uses `winget` and installs LLVM tools plus cppcheck when missing.
